 ...
 *
 * v9:
 * - [FITUR] Persistensi Cycle: Menyimpan device_mode dan crop_cycle_id ke NVS.
 * - Jika perangkat restart saat mode "active", perangkat akan otomatis
 * melanjutkan cycle tersebut saat boot, termasuk memulai
 * timer 'sensorWarmUpTime' (permintaan user).
 * - Menambah key NVS: KEY_MODE, KEY_CYCLE_ID.
 * - Menambah fungsi saveCycleState().
 * - Modifikasi loadSettings() untuk memulihkan state.
 * - Modifikasi handler start_cycle/stop_cycle (MQTT & WS) untuk menyimpan state.
 *
 * v8:
 * - Indentasi distandarisasi menggunakan 4 spasi.
 * - Fungsi confirm() di HTML diganti dengan pengiriman perinAtah langsung.
 *
 * v7:
 * - [PERBAIKAN] Menambahkan flag 'tdsBufferReady' untuk mencegah error TDS palsu
 * segera setelah periode pemanasan sensor selesai.
 */
...
const char* mqtt_user = "Hidro_2";
const char* mqtt_pass = "Tult0612_labiottel-U";

const char* web_password = "admin";

#define PH_PIN 4 //4
#define TdsSensorPin 5 // 5
#define ONE_WIRE_BUS 20 // ubah jadi 20
const int RELAY_CH1_PIN = 15; // 15
...
const long mixingInterval = 600000;
...
// [TAMBAHAN v9] Key NVS untuk menyimpan status cycle
#define KEY_MODE "deviceMode"
#define KEY_CYCLE_ID "cycleId"
...
<body>
    ...
    <script>
        ...
        function sendStartCycle() {
            ...
            // PERHATIAN: Fungsi confirm() dinonaktifkan sesuai aturan lingkungan.
            // Perintah akan langsung dikirim.
            addToLog(`Memulai cycle baru: ${crop_cycle_id}. Mengirim perintah...`);
            sendCommand({
                action: "start_cycle",
                crop_cycle_id,
                ph_min, ph_max,
        ...
        function sendStopCycle() {
            // PERHATIAN: Fungsi confirm() dinonaktifkan sesuai aturan lingkungan.
            // Perintah akan langsung dikirim.
            addToLog("Menghentikan siklus saat ini. Mengirim perintah...");
        ...
        function sendSetWifi() {
            ...
            // PERHATIAN: Fungsi confirm() dinonaktifkan sesuai aturan lingkungan.
            // Perintah akan langsung dikirim.
            addToLog(`Mengganti WiFi ke: ${ssid}. Mengirim perintah restart...`);
        ...
        function sendResetWifi() {
            // PERHATIAN: Fungsi confirm() dinonaktifkan sesuai aturan lingkungan.
            // Perintah akan langsung dikirim.
            addToLog("MENGHAPUS SEMUA WiFi tersimpan. Mengirim perintah restart...");
...
/**
 * @brief [TAMBAHAN v9] Menyimpan status cycle (mode & ID) ke NVS.
 */
void saveCycleState() {
    preferences.begin(NVS_NAMESPACE, false);
    preferences.putString(KEY_MODE, device_mode);
    preferences.putString(KEY_CYCLE_ID, crop_cycle_id);
    preferences.end();
    Serial.print("Status cycle DISIMPAN ke NVS. Mode: "); Serial.print(device_mode);
    Serial.print(", ID: "); Serial.println(crop_cycle_id);
}
...
/**
 * @brief [MODIFIKASI v9] Memuat kalibrasi, threshold, DAN status cycle terakhir.
 */
void loadSettings() {
    ...
    // [TAMBAHAN v9] Memuat status cycle terakhir
    // Baca status mode, default-nya "idle" jika tidak ada
    String savedMode = preferences.getString(KEY_MODE, "idle");
    strncpy(device_mode, savedMode.c_str(), sizeof(device_mode) - 1);
    device_mode[sizeof(device_mode) - 1] = '\0'; // Pastikan null-terminated
    
    if (strcmp(device_mode, "active") == 0) {
        // Jika mode terakhir adalah 'active', muat juga cycle_id
        String savedCycleId = preferences.getString(KEY_CYCLE_ID, "");
        strncpy(crop_cycle_id, savedCycleId.c_str(), sizeof(crop_cycle_id) - 1);
        crop_cycle_id[sizeof(crop_cycle_id) - 1] = '\0'; // Pastikan null-terminated
        
        Serial.println("================================");
        Serial.println("BOOT RECOVERY: Mode 'active' terdeteksi saat boot.");
        Serial.print("Melanjutkan Cycle ID: "); Serial.println(crop_cycle_id);
        
        // [PENTING] Langsung mulai pemanasan sensor, sama seperti start_cycle
        cycleStartTime = millis(); 
        tdsValue = 0;
        phValue = 7.0;
        tdsBufferReady = false;
        Serial.printf("Memulai jeda pemanasan sensor %ld detik...\n", sensorWarmUpTime / 1000);
        Serial.println("================================");
    } else {
        // Jika mode "idle" atau lainnya, pastikan cycle_id kosong
        strcpy(crop_cycle_id, "");
        Serial.println("Boot normal. Mode: idle.");
    }
...
// untuk ngecek setup, berhasil atau ngga
void setupMqtt() {
            ...
            if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
                ...
                if (action && strcmp(action, "RESET_WIFI") == 0) {
                    ...
                    if (doc.containsKey("crop_cycle_id") && doc.containsKey("ph_min") && doc.containsKey("ph_max") && doc.containsKey("ppm_min") && doc.containsKey("ppm_max")) {
                        ...
                        saveCycleState(); // [TAMBAHAN V9] Simpan status baru
                    ...
                    saveCycleState(); // [TAMBAHAN V9] Simpan status baru (idle)
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n================================");
    Serial.println("Memulai Perangkat Hidroponik (v9 - Cycle Persistence)...");

    loadSettings(); // Memuat kalibrasi, threshold, DAN status cycle terakhir
    setupPins();
    setupWifi();
    setupNtp();
    setupMqtt();
...
void printStatus() {
    ...
    Serial.println("--- Status Cycle Saat Ini (RAM) ---");
    Serial.print("  Mode: "); Serial.println(device_mode);
    Serial.print("  Cycle ID: "); Serial.println(strlen(crop_cycle_id) > 0 ? crop_cycle_id : "--");
    Serial.println("-----------------------------------");
}
...
void loop() {
    if (flagResetWifi) {
        ...
        preferences.remove(KEY_PASS);
        preferences.remove(KEY_WIFI_ATTEMPT);
        // [TAMBAHAN V9] Hapus juga status cycle saat reset wifi
        preferences.remove(KEY_MODE);
        preferences.remove(KEY_CYCLE_ID);
        preferences.end();
        delay(1000);
        ESP.restart();
    }
    ...
    if (strcmp(device_mode, "idle") == 0) {
        ...
        tdsErrorState = tdsBufferReady && (tdsValue <= 0);
        ...
        bool anySensorError = tempErrorState || phErrorState || tdsErrorState;
